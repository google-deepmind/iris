...
class AdaptationOptimizersTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ('_batch', maml_worker.HillClimbAdaptationType.BATCH),
      ('_average', maml_worker.HillClimbAdaptationType.AVERAGE),
  )
  ...
  def test_hillclimb_iteration_step(self, parallel_alg):
    ...
    if parallel_alg is maml_worker.HillClimbAdaptationType.BATCH:
      potential_best_params, potential_pivot_value, eval_results = (
          adaptation._batch_iteration_step(
              params_to_eval=self.init_params, work_fn=self.worker_obj.work
          )
      )
    elif parallel_alg is maml_worker.HillClimbAdaptationType.AVERAGE:
      potential_best_params, potential_pivot_value, eval_results = (
...
class MamlWorkerTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ('_hillclimb', maml_worker.HillClimbAdaptation),
      ('_gradient', maml_worker.GradientAdaptation),
  )
