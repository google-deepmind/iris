...
class NormalizerTest(absltest.TestCase):
  ...
  def test_action_range_denormalizer(self):
    ...
    value = {'sensor1': np.array([1, 1, -1, -1, 1]),
  ...
  def test_observation_range_normalizer(self):
    ...
    norm = normalizer.ObservationRangeNormalizer(space,
                                                 ignored_keys=['sensor2'])
    value = {'sensor1': np.array([5, 5, 0, 0, 5]),
  ...
  def test_running_mean_std_normalizer(self):
    ...
    np.testing.assert_array_equal(norm.buffer.mean, value['sensor1'])
    self.assertEqual(norm.buffer.n, 3)
    ...
    np.testing.assert_array_equal(norm.buffer.mean, value['sensor1']*(7/8))
    self.assertEqual(norm.buffer.n, 4)
  ...
  def test_mean_std_buffer_scalar(self):
    ...
    self.assertEqual(mean_std_buffer.std, 1.0)  # First value is always 1.0.

    mean_std_buffer.push(np.asarray(11.0))
    # sqrt(11.0-10.0 / 2.0)
    np.testing.assert_almost_equal(mean_std_buffer.std, np.sqrt(0.5))
  ...
  def test_mean_std_buffer_reject_infinity_on_merge(self):
    ...
    self.assertEqual(mean_std_buffer.n, 1)  # Still only 1 value.
